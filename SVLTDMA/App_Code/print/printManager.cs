...
public class printManager
{
    ...
    static int maxPriceLength = 14;
    static int maxQuantityLength = 5;
    static int maxServiceNameLength = 28;
    ...
    //  Constructors
    /// <summary>
    /// This Constructor is used for all Old Orders for which only 1 printer is supported.
    /// </summary>
    public printManager()
    {
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    /// <summary>
    /// This Constructor is used for all new Orders for which can have one printer per client.
    /// </summary>
    public printManager(string port, string settings)
    {
        //Set the print settings of the class
        portName = port;
    ...
    public void printReceipt(Order o)
    {
        ...
        if (o != null)
        {
            devoteeInfo di = new devoteeInfo(o.devoteeID.ToString());
            ...
            sbReceipt.Append("Name: " + di.firstName + " " + di.lastName + "\n");
             ...
             */
            sbReceipt.Append("Payment Method: " + o.paymentMethodName + "\n");
            sbReceipt.Append("Payment Status: " + o.paymentStatus + "\n");
            if (o.paymentMethodName == "CREDIT CARD")
            {
                sbReceipt.Append("Transaction ID: " + o.authorizationTransactionId + "\n");
                sbReceipt.Append("Card Type: " + o.cardType + "\n");
                sbReceipt.Append("Card Number: " + o.cardNumberMasked + "\n");
                sbReceipt.Append("Order Date: " + o.orderDate + "\n");
                sbReceipt.Append("Approval Code: " + o.authorizationTransactionCode + "\n");
            }
            if (o.paymentMethodName == "CHECK")
            {
                sbReceipt.Append("Check Number: " + o.checkNumber + "\n");
                sbReceipt.Append("Check Date: " + o.checkDate + "\n");
            }
            if (o.orderStatus == "Completed")
            {
                sbReceipt.Append("------------------------------------------------\n");
                sbReceipt.Append("\x1b\x45");                                            //Select Emphasized Printing - Pg. 3-14 in the linemode_cm_en.pdf
                sbReceipt.Append(processServiceItemDisplay("Service Name", "Qty", "Price") + "\n");
                sbReceipt.Append("\x1b\x46");                                            //Cancel Emphasized Printing - Pg. 3-14 in the linemode_cm_en.pdf
                sbReceipt.Append("------------------------------------------------\n");
                List<string> serviceDates = o.OrderItems.Select(i => i.serviceDate).Distinct().ToList();
                foreach (var sd in serviceDates)
                {
                    sbReceipt.Append("\x1b\x45");
                    sbReceipt.Append("Service Date: " + sd.ToString() + "\n");
                    sbReceipt.Append("\x1b\x46");
                    foreach (var item in o.OrderItems.Where(x => x.serviceDate == sd).ToList())
                    {
                        sbReceipt.Append(processServiceItemDisplay(item.serviceName, string.Format("{0:N0}", item.quantity), string.Format("{0:C}", item.price)) + "\n");
                    }
                }
                sbReceipt.Append("------------------------------------------------\n");
                sbReceipt.Append("\x1b\x45");                                            //Select Emphasized Printing - Pg. 3-14 in the linemode_cm_en.pdf
                sbReceipt.Append(processServiceItemDisplay("Total: ", " ", string.Format("{0:C}", o.orderTotal)) + "\n");
                sbReceipt.Append("\x1b\x46");                                            //Cancel Emphasized Printing - Pg. 3-14 in the linemode_cm_en.pdf
                sbReceipt.Append("------------------------------------------------\n");
            }
            ...
            {
                sbReceipt.Append("\x1b\x45");                                            //Select Emphasized Printing - Pg. 3-14 in the linemode_cm_en.pdf
                sbReceipt.Append("Refund Amount: " + string.Format("{0:C}", o.orderTotal) + "\n\n");
                sbReceipt.Append("\x1b\x46");                                            //Cancel Emphasized Printing - Pg. 3-14 in the linemode_cm_en.pdf
            }
    ...
    private string processServiceItemDisplay(string serviceName, string quantity, string price)
    {
        if(serviceName.Length < maxServiceNameLength)
        {
            while(serviceName.Length < maxServiceNameLength)
            {
                serviceName += " ";
            }
        } 
        ...
        {
            while (quantity.Length < maxQuantityLength)
            {
                quantity = " " + quantity;
            }
        }
        ...
        {
            quantity = (quantity).Substring(0, maxQuantityLength);
        }
        if (price.Length < maxPriceLength)
        {
            while (price.Length < maxPriceLength)
            {
                price = " " + price;
            }
        }
        ...
        {
            price = (price).Substring(0, maxPriceLength);
        }
    ...
    public void printReceipt(DevoteeDTO di, ServiceRequestDTO sr, paymentResponse res, string printRequestType)
    {
        if (sr != null)
        {
            ...
            sbReceipt.Append("Service Date: " + String.Format("{0:MM/dd/yyyy}", Convert.ToDateTime(sr.Service_Date)) + "\n");
    ...
    public void printReceipt(string serviceName, string serviceAmount)
    {
        DateTime dt = new DateTime();
        dt.Date.ToString();
        string receipt = "\x1b\x1d\x61\x0" +                    //Center Alignment - Refer to Pg. 3-29 in the linemode_cm_en.pdf
        "\x1B\x1C\x70\x1\x0" +                                  //Stored Logo Printing - Refer to Pg. 3-38 in the linemode_cm_en.pdf
        institutionName + "\n" +
        institutionAddress + "\n\n" +
        "\x1b\x1d\x61\x0" +                                     //Left Alignment - Refer to Pg. 3-29 in the linemode_cm_en.pdf
    ...
    public Boolean testPrinterStatus()
    {
        ...
        try
        {
            port = StarMicronics.StarIO.Factory.I.GetPort(portName, portSettings, 3 * 1000);
            ret = true;
        }
